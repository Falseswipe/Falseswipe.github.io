...
function textOptionsMenu(categories) {
  ...
  clearScreen(function() {
    ...
    if (categories.size) {
      ...
      if (oldZoom <= 0.5)="" {="" options[options.length-1].unselectable="true;" }="" if="" (oldzoom="" !="=" 1)="" options.push({name:"reset="" the="" text="" to="" its="" original="" size.",="" group:"choice",="" reset:true});="" (categories.color)="" options.push(="" {name:"使用黑色背景。",="" color:"black"},="" {name:"使用淡黄色背景。",="" color:"sepia"},="" {name:"使用白色背景。",="" color:"white"}="" );="" (categories.animation)="" {name:="" "启用翻页效果。",="" animation:1},="" "禁用翻页效果。",="" animation:2}="" printoptions([""],="" options,="" function(option)="" (option.resume)="" return="" clearscreen(function()="" setbuttontitles();="" loadandrestoregame();="" });="" else="" (option.color)="" changebackgroundcolor(option.color);="" (option.reset)="" setzoomfactor(1);="" (option.animation)="" window.animateenabled="option.animation" 2;="" (initstore())="" store.set("preferredanimation",="" parsefloat(option.animation));="" changefontsize(option.bigger);="" textoptionsmenu(categories);="" })="" curl();="" function="" getzoomfactor()="" (document.body.style.zoom="==" undefined)="" window.zoomfactor="" ||="" 1;="" var="" zoomfactor="parseFloat(document.body.style.zoom);" (isnan(zoomfactor))="" zoomfactor;="" setzoomfactor(zoomfactor)="" initialmaxwidth="680;" document.body.style.maxwidth="(initialMaxWidth" zoomfactor)="" +="" "px";="" document.body.style.transformorigin="center top" ;="" document.body.style.transform="scale(" +zoomfactor+")";="" document.body.style.webkittransformorigin="center top" document.body.style.webkittransform="scale(" document.body.style.zoom="zoomFactor;" store.set("preferredzoom",="" string(zoomfactor));="" changefontsize(bigger)="" oldzoom="getZoomFactor();" (bigger)="" setzoomfactor(oldzoom="" 0.1);="" -="" changebackgroundcolor(color)="" (color="==" "sepia")="" document.body.classlist.remove("nightmode");="" document.body.classlist.remove("whitemode");="" "black")="" document.body.classlist.add("nightmode");="" "white")="" document.body.classlist.add("whitemode");="" store.set("preferredbackground",="" color);="" isnightmode()="" document.body.classlist.contains("nightmode");="" spell(num)="" (num=""> 99) return num;
...
function printAchievements(target) {
  ...
  for (var i = 0; i < totalAchievements; i++) {
    ...
    buffer.push("<b>");
    buffer.push(achievement.title.replace(/&/g, '&amp;').replace(//g, '&gt;').replace(/"/g, '&quot;'));
    buffer.push(":");
    buffer.push("</b> ");
    buffer.push(description.replace(/&/g, '&amp;').replace(//g, '&gt;').replace(/"/g, '&quot;')
      .replace(/\[b\]/g, '<b>')
      .replace(/\[\/b\]/g, '</b>')
      .replace(/\[i\]/g, '<i>')
      .replace(/\[\/i\]/g, '</i>')
  ...
  if (achievedCount === 0) {
    if (hiddenCount === 0) {
      ...
      buffer = ["你还没有解锁成就。还有"+spell(totalAchievements)+"个成就可解锁（包括一个隐藏成就），共计"+totalScore+"分。</p><p>"];
    } else if (hiddenCount == totalAchievements) {
      buffer = ["你还没有解锁成就。还有"+spell(totalAchievements)+"个隐藏成就可解锁，共计"+totalScore+"分。</p><p>"];
    } else {
      buffer = ["你还没有解锁成就。还有"+spell(totalAchievements)+"个成就可解锁（包括 "+spell(hiddenCount)+"个隐藏成就，共计"+totalScore+"分。</p><p>"];
    }
    ...
    if (totalAchievements == 2) {
      buffer = ["恭喜！你已经解锁了全部成就，共得分"+score+"。</p><p>"];
    } else {
      buffer = ["恭喜！你已经解锁了全部的"+spell(totalAchievements)+"个成就，共得分 "+score+"。</p><p>"];
    }
    buffer.push.apply(buffer, unlockedBuffer);
    buffer.push("</p><p>");
  } else {
    buffer = ["你已经在"+spell(totalAchievements)+"个成就中解锁了"+spell(achievedCount)+"个，满分"+totalScore+"，共计得分"+score+"。</p><p>"];
    buffer.push.apply(buffer, unlockedBuffer);
    var remaining = totalAchievements-achievedCount;
    if (remaining == hiddenCount) {
      if (remaining == 1) {
        buffer.push("</p><p> 还有一个隐藏成就未解锁</p><p>");
      } else {
        buffer.push("</p><p>还有" + spell(remaining) + "个隐藏成就未解锁。</p><p>");
      }
    } else if (hiddenCount > 1) {
      buffer.push("</p><p>还有 "+spell(remaining)+"个成就未解锁，包括"+spell(hiddenCount)+"个隐藏成就.</p><p>");
    } else if (hiddenCount == 1) {
      buffer.push("</p><p>还有 "+spell(remaining)+"个成就未解锁，包括一个隐藏成就.</p><p>");
    } else if (remaining == 1) {
      buffer.push("</p><p>还有一个成就未解锁。</p><p>");
    } else {
      buffer.push("</p><p>还有"+spell(remaining)+"个成就未解锁。</p><p>");
    }
    if (lockedBuffer.length) {
      buffer.push.apply(buffer, lockedBuffer);
      buffer.push("</p><p>");
...
function printShareLinks(target, now) {
  ...
  if (window.isAndroidApp) {
    ...
    if (window.isNookAndroidApp) {
      if (window.nookEan && nookEan != "UNKNOWN") {
        mobileMesg = "  <li><a href="choiceofgamesnook://" +window.nookean+""="" target="_blank" rel="external">Rate this app</a> in the Nook App Store</li>\n";
      }
    } else if (androidLink) {
      androidUrl = androidLink.href;
      if (androidUrl) {
  ...
  var shareLinkText = '<li>'+(dataUriSupported?'<img height="16" width="16" src="data:image/png;base64,">':"")+
        ' <a href="http://www.stumbleupon.com/submit?url='+url+'&amp;title='+title+'" class="spacedLink" target="_blank" rel="external">StumbleUpon</a></li>'+
        ...
        ' <a href="http://www.facebook.com/sharer.php?u='+url+'&amp;t='+title+'" '+="" 'onclick="if (window.isFile || window.isXul) return true; '+
        'window.open(&quot;http://www.facebook.com/sharer.php?u='+url+'&amp;t='+title+
        '&quot;,&quot;sharer&quot;,&quot;toolbar=0,status=0,width=626,height=436&quot;);return false;" class="spacedLink" target="_blank" rel="external">Facebook</a></li>'+
        ...
        ' <a href="https://twitter.com/intent/tweet?related=choiceofgames&amp;text=Awesome+game%3A+'+title+'&amp;url='+url+'&amp;via=choiceofgames" class="spacedLink" target="_blank" rel="external">Twitter</a></li>';
  ...
  fetchEmail(function(defaultEmail) {
    ...
    if (!errorMessage) errorMessage = "";
      ...
      if (optional == optional_start) {
        ...
        form.innerHTML = "<div id="message" style="color:red; font-weight:bold">"+errorMessage+
          "</div><span><span>My email address is: </span><input type="email" name="email" id="email" value="" +="" escapedemail+""="" style="font-size: 25px; width: 12em"></span><p><label class="noBorder" id="subscribeLabel" for="subscribe">"+
          ...
          recordEmail(email, function() {
            if ("yes" == choice) {
              ...
              form.innerHTML = "<div id="message" style="color:red; font-weight:bold"></div>"+
                "<div>My email address is: </div><div><input type="email" name="email" id="email" value="" +="" escapedemail+""="" style="font-size: 25px; width: 12em"></div>"+
...
window.onload=function() {
    ...
    if (isFollowEnabled()) {
      ...
      '&amp;layout=button_count&amp;width=90&amp;show_faces=true&amp;font&amp;colorscheme=light&amp;action=like&amp;height=20&amp;appId=190439350983878" '+="" '="" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:90px; height:20px;" allowtransparency="true"></iframe>'+
      '<iframe allowtransparency="true" frameborder="0" scrolling="no" '+="" 'src="//platform.twitter.com/widgets/follow_button.html?screen_name=choiceofgames&amp;show_screen_name=false" '="" style="width:160px; height:20px;"></iframe>';
    }
